...
const LocCountrySelect = ({ value, onChange }) => {
  ...
  @media(max-width:768px){
    .contacts-grid4,.contacts-grid3{grid-template-columns:repeat(2,1fr)!important;}
    .contacts-grid2{grid-template-columns:1fr!important;}
    .contacts-split{flex-direction:column!important;}
    .contacts-sidebar{width:100%!important;min-width:unset!important;max-width:unset!important;}
    .contacts-panel{width:100%!important;}
    .contacts-table{overflow-x:auto;-webkit-overflow-scrolling:touch;}
    .contacts-form-row{grid-template-columns:1fr!important;}
    .contacts-hide{display:none!important;}
  }
  @media(max-width:480px){
    .contacts-grid4,.contacts-grid3,.contacts-grid2{grid-template-columns:1fr!important;}
  }
`}</style>
  ...
  return (
    <DashboardLayout title="Contacts">
      ...
      <div id="contacts-split-container" style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {showCreateForm && (
          <div className="contact-create-panel" style={{ flex: `0 0 ${detailPanelWidth}%`, background: 'white', borderRight: '1px solid #e0e0e0', display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
            <style>{`
              @media (max-width: 768px) {
              ...
              <>
                {viewMode === 'grid' ? (
                  ...
                  const activeCols = [
                    ...
                    ...allFieldDefs
                      .filter(f=>f.isActive&&f.fieldName!=='firstName'&&f.fieldName!=='lastName')
                      .sort((a,b)=>a.displayOrder-b.displayOrder)
                      .map(f=>{
                        // Check if standard field with mapping
                        if (FIELD_COL_MAP[f.fieldName]) {
                          return { key:f.fieldName, ...FIELD_COL_MAP[f.fieldName] };
                        }
